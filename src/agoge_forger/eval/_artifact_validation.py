...
def _require_merged_model_layout(
    ...
    _require_merged_tensor_schema(indexed, model_weights, config)
...
def _require_merged_tensor_schema(
    indexed: IndexedArtifacts,
    ...
    problems = (missing, unexpected, wrong_shapes)
    if any(problems):
        raise ValueError(
...
def _read_merged_tensor_schema(
    ...
    with tempfile.TemporaryDirectory(prefix="agoge-model-schema-") as snapshot_dir:
        for index, portable in enumerate(sorted(model_weights)):
            ...
            try:
                _collect_tensor_schema(snapshot, portable, schema)
            finally:
...
def _expected_merged_tensor_schema(config_payload: dict[str, object]) -> dict[str, tuple[int, ...]]:
    model = _empty_causal_lm(config_payload)
    expected = {name: tuple(tensor.shape) for name, tensor in model.state_dict().items()}
    _drop_omitted_model_keys(expected, model)
    return expected
...
def _drop_omitted_model_keys(expected: dict[str, tuple[int, ...]], model: object) -> None:
    _drop_tied_model_keys(expected, model)
    _drop_ignored_model_keys(expected, model)


def _drop_tied_model_keys(expected: dict[str, tuple[int, ...]], model: object) -> None:
...
def _drop_ignored_model_keys(expected: dict[str, tuple[int, ...]], model: object) -> None:

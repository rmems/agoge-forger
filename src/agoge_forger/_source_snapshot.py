...
def _copy_source_bytes(source: Path, snapshot: Path) -> tuple[str, _SnapshotIdentities]:
    ...
    return digest.hexdigest(), _SnapshotIdentities(
        identity_before, identity_after, path_identity_after
    )
...
def _require_unchanged_source(
    source: Path,
    copied: str,
    identities: _SnapshotIdentities,
) -> None:
    if (
        identities.identity_before != identities.identity_after
        or identities.identity_after != identities.path_identity_after
    ):

...
The whole Python test suite is CPU-only, network-free, and runs in a few seconds (~110 tests). It never downloads models: tests use hand-rolled `Dummy*` stand-ins, `tmp_path` fixtures, and — in `test_trainer_trl_api.py` — a model built from a bare `LlamaConfig` plus an in-memory word-level tokenizer. Prefer keeping it that way. If coverage genuinely needs a GPU or the Hub, add it to the manual smoke-test workflows (`docs/ci_smoke_tests.md`) rather than loosening `tests/` — the PR gate runs on a CPU runner with no secrets, so a `tests/` case that needs either will simply fail there.
